...
class Emitter(filename:String) {
	...
	def emitNEWARRAY(in: Type) ={
		in match {
			case (IntType|FloatType|BoolType) => jvm.emitNEWARRAY(getFullType(in))
			case StringType=> jvm.emitANEWARRAY(getFullType(in))
			case _ => throw IllegalOperandException(in.toString)
		}
	}
	...
	def emitREADVAR(name:String,inType:Type,index:Int,frame:Frame) =
	{
		...
		inType match {
			...
			case (ArrayPointerType(_)|ClassType(_)|StringType | ArrayType(_,_)) => jvm.emitALOAD(index)
	...
	def emitWRITEVAR(name:String,inType:Type,index:Int,frame:Frame) =
	{
		...
		inType match {
			...
			case (ArrayPointerType(_)|ClassType(_)|StringType | ArrayType(_,_)) => jvm.emitASTORE(index)
   ...
   */
	...
	def emitMULOP(lexeme:String, in:Type,frame:Frame) ={
		...
		frame.pop();
		if (lexeme.equals("*")) {
			if (in == IntType) jvm.emitIMUL();
			else jvm.emitFMUL();
		}
		else{
			if (in == IntType) jvm.emitIDIV();
			else jvm.emitFDIV();
		}
	}
	...
	def emitRETURN(in:Type,frame:Frame) =
	{
		in match {
			...
			case ArrayPointerType(_)|StringType =>jvm.emitARETURN()
